    ...
    <section className="mx-auto max-w-6xl px-6 py-16">
      ...
      {collection && (
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          {items.map((item) => (
            <article key={item.id} className="milk-card p-6">
              <p className="text-xs font-bold uppercase tracking-wider text-brand-700">{item.date || 'Church community'}</p>
              <h2 className="mt-2 font-serif text-2xl font-bold text-brand-900">{item.title}</h2>
              <p className="mt-2 text-stone-600">{item.description}</p>
            </article>
          ))}
          {db && !items.length && <p className="text-stone-600">Nothing has been shared here yet. Please check back soon.</p>}
        </div>
      )}
    </section>
  )
